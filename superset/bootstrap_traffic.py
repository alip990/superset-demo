"""Traffic-police dashboard on the restored beta backup (backup-db, schema bi - see backups/02-bi.sql).

Creates the DB connection, dataset, charts, a tabbed dashboard with native filters, and
company-tree RLS: a user linked to a company (bi.company_user) sees that company and every
company below it (bi.company_closure).
"""
import json, os
import requests
...
ds = find("dataset", "table_name", "violation") or api("POST", "dataset/", json={
    "database": db_id, "schema": "bi", "table_name": "violation"})["id"]
...
def kpi(name, m, fmt=",d"):
    return chart(name, "big_number_total", {"metric": m, "y_axis_format": fmt, "header_font_size": 0.4,
...
charts = {
    ...
    "gauge": chart("شاخص تخلف (درصد)", "gauge_chart", {
    ...
    # ---- company / overview ----
    "sunburst": chart("ساختار سازمانی: کشور ← استان ← وضعیت", "sunburst_v2", {
    ...
    "status": chart("سهم وضعیت پایش‌ها", "pie", {
    ...
    "company_bar": chart("مقایسه پلیس استان‌ها", "echarts_timeseries_bar", {
    ...
    "daily": chart("پایش‌ها و تخلفات روزانه (شمسی)", "echarts_timeseries_bar", {
    ...
    "hourly": chart("الگوی ساعتی تخلفات", "echarts_area", {
    ...
    "day_hour": chart("نقشه حرارتی روز × ساعت", "heatmap_v2", {
    ...
    # ---- maps ----
    "hex": chart("نقشه سه‌بعدی کانون‌های تخلف", "deck_hex", {
    ...
    "heat": chart("نقشه تراکم پایش‌ها", "deck_heatmap", {
    ...
    "scatter": chart("نقاط تخلف به تفکیک پلیس استان", "deck_scatter", {
    ...
    "roads": chart("پرتخلف‌ترین محورها", "echarts_timeseries_bar", {
    ...
    "treemap": chart("تخلفات: پلیس استان ← محور", "treemap_v2", {
    ...
    "models": chart("تخلفات به تفکیک مدل خودرو", "pie", {
    ...
    "vehicles": chart("خودروهای پرتخلف", "table", {
    ...
    "drivers": chart("رانندگان پرتخلف", "table", {
...
# ---------- dashboard layout: header + KPI row above three tabs ----------
pos = {"DASHBOARD_VERSION_KEY": "v2",
       "ROOT_ID": {"type": "ROOT", "id": "ROOT_ID", "children": ["GRID_ID"]},
       "GRID_ID": {"type": "GRID", "id": "GRID_ID", "children": [], "parents": ["ROOT_ID"]},
       "HEADER_ID": {"type": "HEADER", "id": "HEADER_ID", "meta": {"text": "داشبورد پایش سرعت - پلیس راهور"}}}
...
def row(rid, parents, items):
    ...
    for key, width, height in items:
        if key.startswith("MD:"):
            ...
            add({"type": "CHART", "id": f"CHART-{key}", "children": [],
                 "meta": {"chartId": charts[key], "width": width, "height": height}}, parents + [rid])
...
for tid, title in tabs.items():
    add({"type": "TAB", "id": tid, "children": [], "meta": {"text": title, "defaultText": title}}, T)
...
def select_filter(fid, name, column):
    return {"id": fid, "name": name, "filterType": "filter_select", "type": "NATIVE_FILTER",
            ...
            "chartsInScope": list(charts.values()), "tabsInScope": list(tabs), "description": ""}
...
"""
dash_body = {"dashboard_title": "داشبورد پایش سرعت - پلیس راهور", "slug": SLUG, "published": True,
...
with app.app_context():
    ...
    table = db.session.get(SqlaTable, ds)
    ...
    # current_username() is part of Superset's cache key, so tenants never share cached results
...
print("TRAFFIC_BOOTSTRAP_DONE dashboard_id=", dash_id)
